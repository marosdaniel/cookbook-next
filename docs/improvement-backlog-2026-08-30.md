...
|---|---|---|---|---|---|
| **N-P0-1** ✅ | Adat-integritás | **Saját recept értékelhető**: a [RecipeService.rateRecipe](../src/lib/services/RecipeService.ts#L479) betölti a receptet, de nem ellenőrzi a `createdBy`-t → rating-inflálás. (07-06 óta nyitott.) | **Kész (2026-08-30)**: `rateRecipe` `FORBIDDEN`-t dob, ha `recipe.createdBy === userId`; a `RecipeRating` a saját recepten `readOnly`-ra vált. | **S** | Csak kód, nincs függőség |
...
| **N-P2-3** ✅ | SEO | Nincs RSS/Atom feed. | **Kész (2026-08-30)**: [src/app/feed.xml/route.ts](../src/app/feed.xml/route.ts) — legújabb 20 recept, `Cache-Control: s-maxage=3600`. | **S** | Beépített route handler |
